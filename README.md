...
<p align="center">
  用户注册 · 图片额度 · 每日签到 · 会员套餐 · 兑换码 · 优惠码 · 邮箱验证 · 账号池 · 网页生图
</p>
...
- 每日签到可按后台配置给用户发放普通图片额度奖励，同一用户同一自然日只能领取一次。
...
用户登录后可以每天签到领取图片额度奖励。签到记录按自然日去重，同一用户每天只能成功签到一次；奖励会写入普通图片额度，可用于后续网页生图。

管理员可在后台配置是否启用签到、每日普通图片额度、连续签到奖励和签到时区。签到记录保存在现有运行时数据库 `data/users.db` 的 `daily_checkins` 表中，与用户、额度等数据共用同一个用户数据库；备份或迁移生产环境时仍需保留 `data/users.db`。
...
- `data/users.db`：用户、额度、每日签到记录（`daily_checkins` 表）、会员套餐、兑换码、优惠码、验证码等数据。
