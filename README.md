...
<p align="center">
  用户注册 · GGB · 每日签到 · 会员套餐 · 兑换码 · 优惠码 · 邮箱验证 · 账号池 · 网页生图
</p>
...
Genapi 是一个以网页生图为核心的自托管系统，目标是把 ChatGPT 官网图片能力包装成可运营、可分发、可控 GGB 的图片生成站点。

它更接近一个“图片生成业务后台”，而不是传统 API 中转服务。当前版本已经关闭 `/v1/*` OpenAI 兼容外部接口，普通用户只能通过网页端使用生图能力。管理员负责维护用户、GGB、会员套餐、兑换码、优惠码、邮箱验证、账号池和系统配置。
...
0.1.16 起，站内货币统一展示为 `GGB`，每张图片消耗 `5 GGB`，并保留旧 API 字段兼容；系统设置中的账号刷新间隔会在运行中生效，账号池支持筛选并排除问题账号。
...
- 管理员可以创建、禁用、删除用户。
- 管理员可以调整用户角色、GGB 余额和图片并发数。
- 普通用户只能访问自己的生图工作台和兑换记录。
- 管理员拥有账号池、用户、码管理、日志、系统设置等后台权限。

### GGB 体系

- 站内货币统一展示为 `GGB`，每张图片消耗 `5 GGB`。
- 普通用户每次生图会先按请求图片数预留 GGB，再按实际返回图片数结算。
- 请求失败或没有返回图片时，会退回未使用的 GGB。
- 普通 GGB 和会员 GGB 分开记录，生图时优先消耗会员 GGB，再消耗普通 GGB。
- 每日签到可按后台配置给用户发放普通 GGB 奖励，同一用户同一自然日只能领取一次。
- 普通用户受图片并发限制，超限会返回 429。
- 管理员不受 GGB 和图片并发限制。
- 文本相关内部能力不扣 GGB。
- 对外响应和管理接口继续兼容旧字段名，例如 `image_quota`、`member_image_quota`、`total_image_quota`、`period_image_quota`、`reward_image_quota`、`default_image_quota` 等；字段值按 GGB 语义处理。

这套规则适合直接做成“次数包”“会员赠送次数”“活动赠送次数”等业务模型。

### 每日签到

用户登录后可以每天签到领取 GGB 奖励。签到记录按自然日去重，同一用户每天只能成功签到一次；奖励会写入普通 GGB，可用于后续网页生图。

管理员可在后台配置是否启用签到、每日普通 GGB、连续签到奖励和签到时区。签到记录保存在现有运行时数据库 `data/users.db` 的 `daily_checkins` 表中，与用户、GGB 等数据共用同一个用户数据库；备份或迁移生产环境时仍需保留 `data/users.db`。
...
- 套餐名称和说明。
- 有效天数，例如日卡、周卡、月卡。
- GGB 周期天数，例如每 7 天刷新一次。
- 每周期会员 GGB。
- 启用状态和排序。

用户可以在 `/membership` 查看当前会员、周期 GGB、周期结束时间、到期时间和可兑换套餐。会员周期从激活时间开始滚动计算，不按自然日、自然周或自然月重置；每个周期到达边界后刷新到套餐 GGB，不结转上个周期剩余 GGB。会员到期后只清空会员 GGB，不影响普通 GGB。用户兑换新的会员码会替换旧会员，并清空旧会员 GGB 后按新套餐重新激活。

### 兑换码

管理员可以批量生成一次性兑换码：

- `image_quota`：兑换 GGB，保留旧字段名用于 API 兼容。
- `concurrency`：提升图片并发数。
- `invitation`：作为注册邀请码使用。
- `membership`：激活或替换会员套餐。

用户登录后可在 `/redeem` 输入兑换码。兑换成功后，普通 GGB、并发或会员套餐立即写入用户账户。同一个码只能使用一次，后台可以查看状态和使用记录。
...
- `data/users.db`：用户、GGB、每日签到记录（`daily_checkins` 表）、会员套餐、兑换码、优惠码、验证码等数据。
...
.
├── api/                 # FastAPI 路由
├── services/            # 用户、GGB、邮箱、账号池、生图等服务逻辑
├── utils/               # 通用工具
├── test/                # 后端测试
├── web/                 # Next.js 前端
├── docs/                # 补充文档
