...
function authorizationHeader(authToken?: string): Record<string, string> | undefined {
...
function proxySettingsFromConfig(config: SettingsConfig): ProxySettings {
  const url = typeof config.proxy === "string" ? config.proxy : "";
  return {
...
export async function fetchProxy() {
  ...
  return { proxy: proxySettingsFromConfig(data.config) };
}

export async function updateProxy(updates: { enabled?: boolean; url?: string }) {
  const current = await fetchSettingsConfig();
  const currentUrl = typeof current.config.proxy === "string" ? current.config.proxy : "";
  const nextUrl = updates.enabled === false ? "" : updates.url !== undefined ? updates.url.trim() : currentUrl;
  const data = await updateSettingsConfig({ proxy: nextUrl });
  return { proxy: proxySettingsFromConfig(data.config) };
}
