...
export function mergeRegisterConfigForPoll(
  current: RegisterConfig | null,
  incoming: RegisterConfig,
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  loadRegister: async (silent = false) => {
    ...
    try {
      ...
      set((state) => ({
        registerConfig: mergeRegisterConfigForPoll(
          state.registerConfig,
          data.register,
          silent && state.registerConfigDirty,
        ),
        registerConfigDirty: silent ? state.registerConfigDirty : false,
      }));
    } catch (error) {
  ...
  setRegisterConfig: (config) => {
    set({ registerConfig: config, registerConfigDirty: false, isLoadingRegister: false });
  },

  setRegisterProxy: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, proxy: value }, registerConfigDirty: true } : {});
  },

  setRegisterTotal: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, total: Number(value) || 0 }, registerConfigDirty: true } : {});
  },

  setRegisterThreads: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, threads: Number(value) || 0 }, registerConfigDirty: true } : {});
  },

  setRegisterMode: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, mode: value }, registerConfigDirty: true } : {});
  },

  setRegisterTargetQuota: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, target_quota: Number(value) || 0 }, registerConfigDirty: true } : {});
  },

  setRegisterTargetAvailable: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, target_available: Number(value) || 0 }, registerConfigDirty: true } : {});
  },

  setRegisterCheckInterval: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, check_interval: Number(value) || 0 }, registerConfigDirty: true } : {});
  },
  ...
  updateRegisterProvider: (index, updates) => {
    set((state) => {
      ...
      return { registerConfig: { ...state.registerConfig, mail: { ...state.registerConfig.mail, providers } }, registerConfigDirty: true };
    });
  },
  ...
  saveRegister: async () => {
    ...
    try {
      ...
      set({ registerConfig: data.register, registerConfigDirty: false });
