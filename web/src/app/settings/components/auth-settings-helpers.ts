...
export function normalizeAuthSettingsForForm(settings: AuthSettings): AuthSettings {
  ...
  return {
    ...
    checkin_enabled: booleanValue(source.checkin_enabled, false),
    checkin_daily_image_quota: integerValue(source.checkin_daily_image_quota, 0),
    checkin_streak_bonus_enabled: booleanValue(source.checkin_streak_bonus_enabled, false),
    checkin_streak_bonus_days: integerValue(source.checkin_streak_bonus_days, 1, 1),
    checkin_streak_bonus_image_quota: integerValue(source.checkin_streak_bonus_image_quota, 0),
    checkin_timezone: textValue(source.checkin_timezone) || "Asia/Shanghai",
...
export function buildAuthSettingsPayload(settings: AuthSettings): Partial<AuthSettings> {
  ...
  const payload: Partial<AuthSettings> = {
    ...
    checkin_enabled: normalized.checkin_enabled,
    checkin_daily_image_quota: normalized.checkin_daily_image_quota,
    checkin_streak_bonus_enabled: normalized.checkin_streak_bonus_enabled,
    checkin_streak_bonus_days: normalized.checkin_streak_bonus_days,
    checkin_streak_bonus_image_quota: normalized.checkin_streak_bonus_image_quota,
    checkin_timezone: normalized.checkin_timezone,
