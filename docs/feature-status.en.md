...
| 注册验证码、邀请码、兑换码、优惠码 | ✅  | 已支持数据库存储、哈希码值、注册准入、狗狗币/并发兑换和会员兑换。 |
| 会员套餐与周期狗狗币 | ✅  | 已支持管理员配置日卡/周卡/月卡等套餐，会员码激活或替换会员，会员狗狗币按激活时间滚动周期刷新且不结转。 |
| 狗狗币站内货币 | ✅  | 0.1.16 起统一展示为 `狗狗币`，每张图片消耗 `5 狗狗币`；旧 API 字段名继续兼容并按狗狗币语义处理。 |
| 账号池管理 | ✅  | 已支持列表、筛选、批量操作、导出、手动编辑、刷新和删除。 |
| 账号额度刷新与恢复时间同步 | ✅  | 已支持账号信息刷新，限流账号也会自动继续检查。 |
| 账号刷新间隔运行中生效 | ✅  | 0.1.16 起系统设置中的账号刷新间隔调整后无需重启即可生效，并会继续刷新、筛选问题账号。 |
...
| 图片尺寸参数 | ✅  | 已支持固定分辨率预设：`1024x1024`、`1536x864`、`864x1536`、`1280x960`、`960x1280`、`1920x1080`、`1080x1920`、`2560x1440`、`1440x2560`、`3840x2160`、`2160x3840`；未指定尺寸仍保留默认生成行为。 |
| 服务端图片 URL 缓存 | ✅  | 已实现。 |
| `rt_token` 刷新 | ❌  | 待实现。 |
